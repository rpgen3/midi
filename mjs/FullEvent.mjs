class Ph {
    constructor(xx, yy, last, x, events, i, tm, ed){
...
class Event {
    constructor(xx, yy, last, x, events, tm, ed){
        ...
        for(let i = 0; i < 4; i++){
            const j = i * 100,
                  last2 = last - j;
            this.body.push(new Ph(xx, yy, last2, x, events.slice(j, j + 100), i, tm, ed));
            if(last2 < 100) break;
        }
    }
    toStr(){
...
export class FullEvent {
    ...
    make(events, {
        ...
        for(let i = 0; i < this.max; i++){
            ...
            arr.push(new Event(x, y, last, i, events.slice(j, j + 400), tm, ed));
            if(last < 400) break;
        }
